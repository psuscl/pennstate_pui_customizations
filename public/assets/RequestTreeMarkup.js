(function (exports) {
  ...
  class RequestTreeMarkup {
    ...
    root(title) {
      ...
      const rootTemplate = document
        .querySelector('#request-tree-root-template')
    ...
    list(parentElementId, level, numBatches) {
      ...
      const listTemplate = document
        .querySelector('#request-tree-children-list-template')
        .content.cloneNode(true);
      ...
      for (let i = 0; i < numBatches; i++) {
        const itemTemplate = document
          .querySelector('#request-tree-batch-placeholder-template')
          .content.cloneNode(true);
    ...
    node(data, level, shouldObserve, parentId = null, offset = null) {
      ...
      const nodeTemplate = document
        .querySelector('#request-tree-node-template')
        .content.cloneNode(true);
      ...
      /* we'll decide what to do with this later */ 
    ...
    expandButton(title) {
      ...
      const btnTemplate = document
        .querySelector('#request-tree-expand-button-template')
        .content.cloneNode(true);
  ...
  exports.RequestTreeMarkup = RequestTreeMarkup;
})(window);
