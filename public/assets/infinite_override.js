(function (exports) {
  class InfiniteTree {
    ...
    nodeRowMarkup(node, level, shouldObserve, parentId = null, offset = null) {
      ...
      nodeRow.querySelector('.table-row').classList.add(`table-row-${node.level}`);
      nodeRow.querySelector('.table-row').setAttribute('data-uri', node.uri);
      nodeRow.querySelector('.review-record').classList.add(`record-${node.level}`);

      if(node.level == "series" || node.level == "subseries") {
        nodeRow.querySelector(`.record-${node.level}`).prepend(`${this.capitalize(node.level)}: `);
      }
     ...
     * @returns{string} str - The capitalized string
     */
    capitalize(str) {
